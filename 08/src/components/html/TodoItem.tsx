...
const TodoItem = ({ task, isCompleted }: TTodoItemProps) => {
  return (
    <li className="flex items-center justify-between border border-[#4F4F4F] h-[44px] px-[15px] rounded-lg bg-[rgba(53,56,62,0.05)] select-none shrink-0">
      <Checkbox checked={isCompleted} onToggle={() => setAgree(!isCompleted)}>
        <span className="line-through text-[#35383E]">{task}</span>
      </Checkbox>
